...
public class ZoomContent : ContentLayout
{
    ...
    */
    ...
    public (SKRect ScaledDestination, float Scale) ComputeContentScale(SKRect destination, SKPoint offsetCenter, float scale)
    {
        ...
        float clampedOffsetX = DrawnExtensions.Clamp(scaledOffset.X, minOffsetX, maxOffsetX);
        float clampedOffsetY = DrawnExtensions.Clamp(scaledOffset.Y, minOffsetY, maxOffsetY);
        ...
        if (touchAction == TouchActionResult.Pinched)
        {
            ...
            if (!ZoomLocked)
            {
                if (_lastPinch != 0)
                {
                    ...
                    {
                        var moved = PointFExtensions.Distance(args.Pinch.Center, _pinchCenter);
