            ...
            if (structure != null)
            {
                ...
                foreach (var cell in visibleElements)
                {
                    ...
                    if (IsTemplated)
                    {
                        ...
                        {
                            Debug.WriteLine("Templates not ready yet for rendering");
                            break; //itemssource was changed by other thread
                        }
                        //binding context set
                        child = ChildrenFactory.GetChildAt(cell.ControlIndex);
                    }
                    else
                    {
                        child = cell.View;
                    ...
                    {
                        //NeedMeasure = true;
                        Debug.WriteLine("Oops got empty control to draw in stack");
                        return drawn;
                    }
        ...
        public class BuildWrapLayout : StackLayoutStructure
        {
            ...
            public override ScaledSize Build(SKRect rectForChildrenPixels, float scale)
            {
                ...
                void LayoutCellsInternal()
                {
                    ...
                    {
                        var layoutMaxHeight = rectFitChild.Height.IsFinite() ? rectFitChild.Height : maxHeight;
                        var layoutHeight = _layout.VerticalOptions.Alignment == LayoutOptions.Fill.Alignment
                            ? layoutMaxHeight : maxHeight;

                        foreach (var controlInStack in cellsToLayoutLater)
                        {
