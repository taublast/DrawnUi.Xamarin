        ...
        protected virtual void UpdateFont()
        {
            ...
            lock (LockFont)
            {
                    ...
                    || TypeFace == null)
                {
                    ...
                    var replaceFont = SkiaFontManager.Instance.GetRegisteredFont(_fontFamily);
    ...
    TextSpan span)
        {
            ...
            foreach (var line in paragraphs)
            {
                ...
                bool AddLine(string adding, string full = null)
                {
                    ...
                    if (!string.IsNullOrEmpty(adding))
                    {
                        ...
                        if (result.Count > 0)
                        {
                            result[result.Count - 1].IsLastInParagraph = addLine.IsNewParagraph;
                        }
            ...
            if (result.Count > 0)
            {
                result[result.Count - 1].IsLastInParagraph = true;
            }
        ...
        [System.ComponentModel.TypeConverter(typeof(FontAttributesConverter))]
