	...
	protected override void Paint(SkiaDrawingContext ctx, SKRect destination, float scale, object arguments)
	{
		...
		if (source != null && !CheckIsGhost())
		{
			...
			if (PaintColorFilter == null)
			{
				...
				{
					SkiaImageEffect.Tint when ColorTint != TransparentColor
					...
					SkiaImageEffect.TSL when BackgroundColor != TransparentColor
						=> SkiaImageEffects.TintSL(BackgroundColor, (float)Saturation, (float)Brightness, this.EffectBlendMode),

					SkiaImageEffect.HSL when BackgroundColor != TransparentColor
